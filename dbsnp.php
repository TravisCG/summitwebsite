<?php
  ...
  function drawNuc($x, $y, $array, $style){
    // Every nucleotide is 45pixel wide and 100pixel tall
    echo('<path class="'.$style.'" d="M ' . (12 * $x + $array[0]) . ' ' . $array[1] . ' ');
  ...
  function drawMotifLogos($conn, $feats){
    ...
    $regend   = $feats["end"] + 1;
    for($i = 0; $i < sizeof($feats["motifs"]); $i++){
      $sql    = "select probA,probC,probG,probT from pfm left join consensus_motif on (motif_id = consensus_motif_motif_id) where jaspar_code = '".$feats["motifs"][$i][10]."' order by position;";
  ...
  function motifView($conn, $feats, $height){
    echo('<svg width="100%" height="'.$height.'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 1200 300" preserveAspectRatio="none">'."\n");
    echo('<rect x="0" y="0" width="100%" height="100%" style="fill:lightblue;storoke:red;" />');
